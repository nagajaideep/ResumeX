...
// node_modules/codemirror-lang-latex/dist/index.js
var VerbContent = 1;
...
var verbTokenizer = new ExternalTokenizer(
  (input, stack) => {
...
var lstinlineTokenizer = new ExternalTokenizer(
  (input, stack) => {
...
var verbatimTokenizer = new ExternalTokenizer(
  (input, stack) => {
...
var literalArgTokenizer = new ExternalTokenizer(
  (input) => {
    for (let offset = 0; ; offset++) {
      const next = input.peek(offset);
...
var spaceDelimitedLiteralArgTokenizer = new ExternalTokenizer(
  (input) => {
    for (let offset = 0; ; offset++) {
      const next = input.peek(offset);
...
var lookaheadTokenizer = (getToken) => new ExternalTokenizer(
  (input) => {
...
var trailingContentTokenizer = new ExternalTokenizer(
  (input, stack) => {
...
var labelCommands = /* @__PURE__ */ new Set(["\\label", "\\thlabel", "\\zlabel"]);
var mathTextCommands = /* @__PURE__ */ new Set(["\\text", "\\tag", "\\textrm", "\\intertext"]);
...
var latexBracketMatching = bracketMatching({
  brackets: "()[]{}"
});
...
function findSectionBoundaries(node) {
  if (!node)
    return null;
  if (node.name === "Book" || node.name === "Part" || node.name === "Chapter" || node.name === "Section" || node.name === "SubSection" || node.name === "SubSubSection" || node.name === "Paragraph" || node.name === "SubParagraph") {
...
//# sourceMappingURL=codemirror-lang-latex.js.map
